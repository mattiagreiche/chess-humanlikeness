from pipeline.parse import load_data
...
for player, player_phases in data.items():
    if len(player_phases[0]) < num_total_moves or len(player_phases[1]) < num_total_moves or len(player_phases[2]) < num_total_moves:
        print(f"Skipping {player} due to insufficient data.")
        continue
    opening_data = pd.DataFrame(player_phases[0], columns=['board', 'move', 'active_elo', 'inactive_elo']).sample(frac=1, random_state=0).reset_index(drop=True)
    mid_data = pd.DataFrame(player_phases[1], columns=['board', 'move', 'active_elo', 'inactive_elo']).sample(frac=1, random_state=0).reset_index(drop=True)
    end_data = pd.DataFrame(player_phases[2], columns=['board', 'move', 'active_elo', 'inactive_elo']).sample(frac=1, random_state=0).reset_index(drop=True)
...
with open('data/final_data.pkl', 'wb') as f:
    ...
    print("Final data saved to data/final_data.pkl")
